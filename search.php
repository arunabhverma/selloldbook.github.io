...
<!-- filter -->
	...
	<ul name="filter">
			...
			<li><a id="asc" href="?search=<?php echo $_GET['search']; ?>&submit=search&filter=asc">Asc by cost</a></li> <li><a id="desc" href="?search=<?php echo $_GET['search']; ?>&submit=search&filter=desc">Des by cost</a></li> <li><a id="norm" href="?search=<?php echo $_GET['search']; ?>&submit=search&filter=norm">Normal Again</a></li>
...
<div class="a">
	...
	$search = $_GET['search'] ??'';
	?>
	<h3 style="color: white">Search result for <i style="text-transform: uppercase;"><?php echo $search;?></i></h3>
	<div class="grid">		
		<?php
			include 'conn.php';
			...
			$q="SELECT * FROM book WHERE name LIKE '$keyword'  ";

			$qa="SELECT * FROM book WHERE name LIKE '$keyword'  ORDER BY rupee ASC ";

			$qd="SELECT * FROM book WHERE name LIKE '$keyword'  ORDER BY rupee DESC";
			...
			// 	echo "no boss";
			// }
			$res = mysqli_num_rows($qurey);
			if ($res == 0) {
				echo "
				<h3 style='color: white'>No Search result found for <i style='text-transform:uppercase'>$search</i></h3>";
			}
